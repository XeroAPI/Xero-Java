...
public class XeroHttpContext {
	...
	final static Logger logger = LoggerFactory.getLogger(XeroHttpContext.class);
	...
	public CloseableHttpClient getHttpClient() throws FileNotFoundException, IOException
	{
		...
		if(logger.isDebugEnabled()){
			...
			logger.debug(userAgentHeader.toString());
			logger.debug("------------------ HEADER : ACCEPT -------------------");
			logger.debug(acceptHeader.toString());
		}
		...
		if(this.ifModifiedSince != null) {
			...
			if(logger.isDebugEnabled()){
				...
				logger.debug(modifiedHeader.toString());
			}
		}
