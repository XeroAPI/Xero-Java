...
public class BankfeedApiFeedConnectionTest {
	...
	public void testGetFeedConnections() throws Exception {
        ...
        FeedConnections response = bankfeedsApi.getFeedConnections(accessToken,xeroTenantId,null, null);      
        ...
        assertThat(response.getItems().get(0).getId().toString(), (equalTo("c0eb97b5-4f97-465a-8268-276513c14396")));
	...
	public void testGetFeedConnection() throws Exception {
        ...
        FeedConnection response = bankfeedsApi.getFeedConnection(accessToken,xeroTenantId,feedConnectionId);      
	...
	public void testDeleteFeedConnection() throws Exception {
		...
		assertThat(response.getItems().get(1).getStatus(), is(equalTo(com.xero.models.bankfeeds.FeedConnection.StatusEnum.REJECTED)));   
        assertThat(response.getItems().get(1).getError().getDetail(), (equalTo("The AccountToken is connected to another Xero Bank Account associated with this bank. This Xero Bank Account belongs to a different Xero Organisation.")));
        assertThat(response.getItems().get(1).getError().getTitle(), (equalTo("Feed connected in different organisation")));
    }
}
